...
class Scheduler:
    def __init__(self, initial_state, transition_dict, label_dict, scheduler_dict, storm=True):
        self.scheduler_dict = scheduler_dict
        self.initial_state = initial_state
        self.transition_dict = transition_dict
        self.label_dict = label_dict
        self.current_state = None
        self.init_label = "init" if storm else "Init"
        self.all_label_coords = [(extract_coords(self.get_pos_outout(o)),o,s) for s,o in self.label_dict.items() if
                                            self.init_label not in o]
...
class StormInterface:
    def __init__(self, destination, model_name, rev_input_dict,num_steps=None, maximize=True):
        self.model_dir = Path("storm")
        ...
        self.storm_property_file = self.create_mc_query()
        #mdp_2_prism_format(self.model, "porl", output_path=self.tmp_mdp_file)

        self.adv_file_name = (self.model_dir.absolute() / f"{model_name}_{destination}.sched")
        self.concrete_tra_file_name = str(self.model_dir.absolute() / f"{model_name}.tra")
        self.concrete_lab_file_name = str(self.model_dir.absolute() / f"{model_name}.lab")
        
        self.property_val = 0
        self.call_storm()
        self.rev_input_dict = rev_input_dict
        if os.path.exists(self.adv_file_name):
            self.parser = StormSchedulerParser(self.adv_file_name, self.concrete_lab_file_name,
                                               self.concrete_tra_file_name, rev_input_dict)
    ...
    def create_mc_query(self):
        ...
        dest_in_file = destination.replace('"','')
        prop_file_name = self.model_dir.absolute() / f"{self.model_name}_{dest_in_file}.prop"
        with open(prop_file_name, "w") as fp:
            fp.write(prop)
    ...
    def call_storm(self):
        ...
        path_to_storm_file = "storm" #aalpy.paths.path_to_prism[:-len(prism_file)]
        #file_abs_path = path.abspath(self.tmp_mdp_file)
        proc = subprocess.Popen(
            ["storm", "--explicit", self.concrete_tra_file_name,self.concrete_lab_file_name, "--buildchoicelab","--buildstateval", "--prop",
             self.storm_property_file, "--exportscheduler", self.adv_file_name],
...
class StormSchedulerParser:
    def __init__(self, scheduler_file, label_file, transition_file,rev_input_dict):
        ...
        self.actions = list(rev_input_dict.values())
        self.rev_input_dict = rev_input_dict
    ...
    def create_labels(self):
        label_dict = dict()
        header_line = self.label_file_content[1]
        label_lines = self.label_file_content[3:]
        header_dict = dict()
        ...
        for l in label_lines:
            split_line = l.split(" ")
            state_id = int(split_line[0])
            label_names = set(map(lambda x : x.strip(),split_line[1:]))
    ...
    def create_transitions(self):
        ...
        for t in transition_lines:
            ...
            action_int = int(split_line[1])
            action = self.rev_input_dict[action_int]
            transitions[source_state].append((prob, action, target_state))
        return transitions

    def parse_scheduler(self):
        #header_lines = self.scheduler_file_content[0]
        transition_lines = self.scheduler_file_content[3:-2]
        scheduler = dict()
        for t in transition_lines:
            split_line = re.split(r"\s+",t)
            source_state = int(split_line[1])
            action = self.rev_input_dict[int(split_line[2])]
