...
def collect_samples(env, unwrapped, mario, n_samples, max_x = 10000, random_prob = 0):
    ...
    while (e := mario.curr_episode) <= n_samples:
        ...
        while True:
            ...
            if random.random() < random_prob:
                action = random.choice(act_indexes)
            else:
...
def main(eval_mode = False, params_file = None):
    ...
    n_samples = params.getint("TRAINING","N_SAMPLES")
    random_prob = params.getfloat("TRAINING","RANDOM_ACT_PROB")
    if "MAX_X_POS" in params["TRAINING"]:
        max_x = params.getint("TRAINING","MAX_X_POS")
    else:
        max_x = 10000
    samples = collect_samples(env, unwrapped_env, mario, n_samples=n_samples, max_x = max_x, random_prob = random_prob)
    ...
    input_dict = {"right" : 0, "jump" : 1}
    util.save_explicit_mdp(model,input_dict,f"mario_{n_samples}_{stage}")
